...
def calculate_directory_to_delete(directory):
    for d in directory.directories:
        calculate_directory_to_delete(d)
        get_file_to_delete(d)
    return


def get_file_to_delete(directory):
    global file_size_to_delete
    if 30000000 - (70000000 - total_dir_size) <= directory.directory_size <= file_size_to_delete:
        file_size_to_delete = directory.directory_size
    return
...
def main():
    ...
    for d in initial_directory.directories:
        global total_dir_size
        total_dir_size += d.directory_size
    print("Total size of all directories: " + str(total_dir_size))
    calculate_directory_to_delete(initial_directory)
    print("Answer for second Question: " + str(file_size_to_delete))
